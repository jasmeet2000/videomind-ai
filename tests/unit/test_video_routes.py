...
from app.api.routes.video import router, _run_ingestion
    ...
    with patch("app.api.dependencies.services.get_ingest_use_case") as mock_get_uc:
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tf:
            ...
            with open(tf.name, "rb") as f:
                response = client.post("/video/upload", files={"file": ("test.mp4", f, "video/mp4")})
...
async def test_run_ingestion_success():
    with patch("app.use_cases.ingest_video.IngestVideoUseCase.execute", new_callable=AsyncMock) as mock_execute:
