...
async def test_search_route_with_mocked_usecase():
    ...
    with (
        patch("app.api.routes.search.get_search_use_case", return_value=mock_use_case, create=True),
        patch("app.api.dependencies.services.get_search_use_case", return_value=mock_use_case),
    ):
        response = client.post(
            "/api/v1/search", json={"query": "test", "video_id": "vid1", "top_k": 5}
        )
...
async def test_chat_route_with_mocked_usecase():
    ...
    with (
        patch("app.api.routes.chat.get_chat_use_case", return_value=mock_use_case, create=True),
        patch("app.api.dependencies.services.get_chat_use_case", return_value=mock_use_case),
    ):
        response = client.post(
            "/api/v1/chat", json={"question": "test question", "video_id": "vid1"}
        )
