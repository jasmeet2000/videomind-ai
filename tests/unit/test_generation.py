...
def test_context_builder_formats_and_truncates():
    ...
    results = [
        ...
        SearchResult(
            ...
            video_id="v1",
            modality="vision",
            text="this is a much longer test sentence that will be truncated",
            score=0.8,
            start_seconds=20,
            end_seconds=25,
            metadata={},
    ...
    assert (
        "[00:20 - 00:25] (VISION): this is a much longer test sentence that will be truncated"
        not in context
    )
...
async def test_chat_with_video_usecase():
    ...
    mock_retriever.search.assert_called_once_with(
        query="test query", query_vector=[0.1, 0.2], video_id="v1", top_k=2
    )
