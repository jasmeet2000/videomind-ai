...
class TestChunker(unittest.TestCase):
    ...
    def test_basic_grouping_without_overlap(self):
        ...
        segments = [
            TranscriptChunk(
                video_id="v", text="a", start_seconds=0.0, end_seconds=0.5, confidence=1.0
            ),
            TranscriptChunk(
                video_id="v", text="b", start_seconds=1.0, end_seconds=1.5, confidence=1.0
            ),
            TranscriptChunk(
                video_id="v", text="c", start_seconds=2.0, end_seconds=2.5, confidence=1.0
            ),
            TranscriptChunk(
                video_id="v", text="d", start_seconds=3.0, end_seconds=3.5, confidence=1.0
            ),
        ]
        chunks = chunk_transcript_chunks(
            segments, max_tokens=2, overlap_tokens=0, token_counter=lambda t: 1
        )
    ...
    def test_overlap_behaviour(self):
        ...
        chunks = chunk_transcript_chunks(
            segments, max_tokens=2, overlap_tokens=1, token_counter=lambda t: 1
        )
