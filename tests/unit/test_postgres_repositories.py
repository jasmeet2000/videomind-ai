...
def mock_asyncpg():
    ...
    @asynccontextmanager
    async def acquire_mock():
        yield mock_conn

    mock_pool.acquire.side_effect = acquire_mock
...
async def test_postgres_video_repository(mock_asyncpg):
    ...
    mock_record = {
        ...
        "filename": "test.mp4", "status": "pending",
...
async def test_postgres_transcript_repository(mock_asyncpg):
    ...
    mock_record = {
        ...
        "start_seconds": 0.0, "end_seconds": None, "modality": "audio"
    }
    mock_asyncpg.fetch.return_value = [mock_record]
    
    fetched = await repo.get_chunks_by_video_id("vid1")
